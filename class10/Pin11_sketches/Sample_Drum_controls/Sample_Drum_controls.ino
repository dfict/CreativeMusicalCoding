...
// Global variables
unsigned long currentSeed = 1;    // Will be updated by random generation
const int seedPin = A2;          // Pot for seed value
const int multiplierPin = A3;    // Pot for multiplier
//const int moduloPin = A4;        // Pot for modulo

unsigned long customRandom() {
  // Read values from pots
  unsigned long seed = map(analogRead(seedPin), 0, 1023, 1, 65535);
  unsigned long multiplier = map(analogRead(multiplierPin), 0, 1023, 1, 4000);
  //unsigned long modulo = map(analogRead(moduloPin), 0, 1023, 1, 32767);
  unsigned long modulo = 32767;
  
  // Generate next value
  currentSeed = (currentSeed * multiplier + seed) % modulo;
...
void loop() {
  ...
  type = customRandom() % 4;  // The % 4 ensures we get values 0-3
  
 // bound = random(9, 17);
    bound = map(analogRead(5),0, 1023, 9, 60);
   tempo = map(analogRead(4), 0, 1023, 10, 2000);
  delay (tempo);
