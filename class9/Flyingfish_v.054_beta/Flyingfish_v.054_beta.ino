...
// simple. count to 24 to make sure you are in tune. never use this to make music! 
//just test your notes this way.
...
//FlyingFlounder floundering to the lowest aspirations of computer music: 
//random. (see m. puckette, 2022)
//so weighted random algorithm works, but is clunky and aleatroic like windchimes.
//leaving it in for posterity's sake
...
void FlyingMarlinTemp(float* noteTable, int phraseBegin, int phraseEnd, float divisor, float mult, float add) 
{
  int j = 1;
  int phraseStop = phraseBegin - 1;
  for (int i = phraseBegin; i > phraseStop; i = i + j) 
  {
    ...
    // Use temperature to affect the mult parameter
    float tempMult = map(mappedTemp, 200, 400, mult * 0.5, mult * 1.5) / 100.0;
    float noiseValue = (perlinNoise((i * tempMult) + floatPot + add) + 1.0) / 2.0;
  ...
  // Print debug information
    ...
    Serial.print(F(" TempMult: "));
    Serial.print(tempMult, 4);
    Serial.print(F(" Note: "));
    ...
    if (digitalRead(BUTTON_PIN) == LOW) {
      Serial.println(F("Button pressed. Exiting FlyingMarlinTemp();"));
      break;
    }
  }
  Serial.println(F("Finished executing FlyingMarlinTemp();"));
}
...
void loop() {
  ...
  switch (mode) {
 case 1:
FlyingMarlinTemp(noteTable, 0, 30, 1, 5.0, 2.5); 
  break;
    case 2:
