...
ISR(TIMER1_COMPA_vect) {
    ...
    phaseInc = map(potValue, 0, 1023, 1, 100);
    ...
    // Generate a simple oscillator based on phase.. why!?!
    int16_t osc = (phase >> 8) - 128; // Convert to signed..
    phase += phaseInc;
    
    // Mix oscillator with noise for interesting textures
    bp2 = (osc/2 + sah/4 - bp2/32 - lp2) / 2 + bp2;
    lp2 = bp2/2 + lp2;
 //    lp2 = bp2/2 + lp2 + sah;// for fun!
    ...
    out = 128 + ((bp2/2 + burst + osc/4 + lowns/64 + lp1/64) / 32);
