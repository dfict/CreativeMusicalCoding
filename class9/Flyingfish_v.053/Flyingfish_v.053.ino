...
void FlyingMarlin(float* noteTable, int phraseBegin, int phraseEnd, float divisor, float mult, float add) 
{
  ...
  for (int i = phraseBegin; i > phraseStop; i = i + j) 
  {
    ...
    //float noiseValue = (rawNoise + 1.0 + add) / 2.0;
    //LET'S check last version. 
    //i'm not super happy with the calculation here 
    //in so many differentstatemetns. also where did add go?
