...
int main()
{
	...
	unsigned char Bass_Left_Adder = 11;
	unsigned char Bass_Right_Adder = 13;
...
//		New_Rand = Random();
		...
		if (Bass_Left_On)
		{
			if (!(--Bass_Left_Sample))
			{
				if ((Bass_Left_Period += Bass_Left_Adder) > Bass_End)
				{
					Bass_Left_On = 0;
				}
		...
		if (Bass_Right_On)
		{
			...
			{
				if ((Bass_Right_Period += Bass_Right_Adder) > Bass_End)
				{
					Bass_Right_On = 0;
				}
		...
		if (!(--Tempo_Outer))
		{
			...
			if (!(--Tempo_Sample))
			{
				...
				Snare_Length = Snare_Length_Start;
